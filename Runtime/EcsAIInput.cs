...
namespace LeoECSLite.UtilityAI {
  public static class EcsAIInput {
    ...
    public static double Has<T>() where T : struct => GetPool<T>().Has(_Entity) ? TRUE : FALSE;
    ...
    private static bool HasPoolsCacheFor(EcsWorld world)
      => _WorldsPoolsCache.TryGetValue(world, out Dictionary<Type, IEcsPool> pools)
      && pools != null;
    ...
    private static IEcsPool GetPool<T>() where T : struct
      => _WorldsPoolsCache[_World].TryGetValue(typeof(T), out IEcsPool pool)
        ? pool
        : CachePool<T>();
